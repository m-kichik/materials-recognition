...
def find_class_for_image(class_embeddings, image_embedding):
    ...
    image_embedding = image_embedding.unsqueeze(0) if image_embedding.dim() == 1 else image_embedding
...
def main():
    ...
    with torch.no_grad(), open(cats_path, "r") as f:
        ...
        all_materials_embeddings = [model.encode_text(clip.tokenize([m[1]]).to(device)) for m in all_materials]
        all_materials_embeddings = torch.stack([emb / emb.norm(dim=-1, keepdim=True) for emb in all_materials_embeddings])
